...
class Slides extends DB_conn {

  public function select() {

    $sql = "select * from slide where status <> 0 order by slideOrder";
  ...
  public function insert($field) {
    ...
    $sql = "insert into slide ($implod_Col) values (:" . $implod_PlaceHolder . ")";
...
//    echo $sql;die;
  ...
  public function selectMinOrder() {
    $sql = "select min(slideOrder) as MinSlideOrder from slide where status <> 0";
    $stmt = $this->connect()->prepare($sql);
    $stmt->execute();
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    return $result['MinSlideOrder'];
  }
...
//       echo $imgID;die();
  ...
  public function update($field, $id) {
   ...
   public function del($id) {
    $sql = "update slide set status = 0 where id = :id";
    $stmt = $this->connect()->prepare($sql);
    $stmt->bindValue(":id", $id);
    $stmt->execute();

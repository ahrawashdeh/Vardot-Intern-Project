...
class Cards extends DB_conn {

  public function select() {

    $sql = "select * from card where status <> 0 order by id desc";
  ...
  public function selectCards() {
    ...
    $sql = "select * from card where status <> 0 limit 6";
    $result = $this->connect()->query($sql);
    if ($result->rowCount() > 0) {
  ...
  public function insert($field) {
    ...
    $sql = "insert into card ($implod_Col) values (:" . $implod_PlaceHolder . ")";
...
//       echo $imgID;die();
  ...
  public function del($id) {
    $sql = "update card set status = 0 where id = :id";
    $stmt = $this->connect()->prepare($sql);
    $stmt->bindValue(":id", $id);
    $stmt->execute();
